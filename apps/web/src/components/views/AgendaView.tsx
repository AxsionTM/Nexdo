    ...
    <div className="flex-1 flex flex-col min-h-0">
      <header className="px-4 py-3 border-b flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground capitalize">{label.sub}</p>
        </div>
        <div className="flex items-center gap-1">
          <Button variant="outline" size="sm" className="h-8 w-8 p-0" onClick={() => setDayOffset((o) => o - 1)}>
            <ChevronLeft className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="sm" className="h-8 px-3" onClick={() => setDayOffset(0)}>
            Сегодня
          </Button>
          <Button variant="outline" size="sm" className="h-8 w-8 p-0" onClick={() => setDayOffset((o) => o + 1)}>
        ...
        <div className="relative px-2 py-2">
          {HOURS.map((hour) => {
            ...
            return (
              <div
                key={hour}
                className={cn('flex gap-3 min-h-[56px]', dragOverHour === hour && 'bg-primary/10 rounded-md')}
                onDragOver={(e) => {
                  e.preventDefault();
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 px-3 py-2 min-w-0">
        ...
        {task.project && (
          <p className="text-[11px] text-muted-foreground mt-0.5 truncate pl-6">{task.project.name}</p>
        )}
      </div>
    </div>
  );
